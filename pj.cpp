...
void game()
{
    ...
    while (temp == 0)
    {
        ...
        for (int i = 0; i < 25; i++) //border game
        ...
        // Info Screen // Display: SPACE SHOOTER, Level, Score, Lives
        arr[4][57] = 'S';
        arr[4][58] = 'P';
        arr[4][59] = 'A';
        arr[4][60] = 'C';
        ...
        // Game Main Loop (Gameplay Mechanics)
        for (int shipy = 24; lives != '0' && level != '3';)
        {
            ...
            // Display Game Grid Each Frame

            // Flicker-free buffered display
            stringstream buffer;
            for (int i = 0; i < 25; i++) {
                for (int j = 0; j < 70; j++) {
                    buffer << arr[i][j];
            ...
            if (level == '1')
            {
                ...
                if (alienscol > 0 && alienscol < 50 && aliensrow <= 23)
                {
                    if (aliensrow != 1)
                    {
                        arr[aliensrow - 1][alienscol] = ' ';//Clears the old alien position before drawing it in the new position.
                    }
                    if (aliensrow != 23) //Puts the alien at its new Y position, still in the same column.
                    {
                        arr[aliensrow][alienscol] = (char)254; // alien
                    }
                    else
                    { // if reached bottom
                        aliensrow = 1;
            ...
            SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), {0, 0});
            // Handle Input (A, D, SPACE)
            if (_kbhit())
            {
                controls = _getch();// read key
                ...
                if (controls == ' ')
                {
                    ...
                    for (int k = bulletx; k >= 0; k--)//Moves up the screen (k--) until it reaches the top (k == 0).
                    {

                        if (k != 22)
                        {
                            arr[k + 1][bullety] = ' ';//Clears the previous bullet trail to avoid leaving a bullet "tail".
                        }
                        if (k != 0)
                        {
                        ...
                        {
                            arr[k][bullety] = (char)219;//end bullet
                        }
                        if (_kbhit())
                        {
                            controls = _getch();
                        ...
                        //Gives illusion of smooth animation.
                        SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), {0, 0});
                    }
                }
            }
            if (aliensrow == 1) //new ailen spawn
            {
                temp1 = 70;
                for (int i = 0; temp1 <= 0 || temp1 >= 50;)
                {
                    ...
                    if (temp1 > 0 && temp1 < 50)
                    {
                        alienscol = temp1;// assign a new random X position for alien
                    }
                }
            }
            SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), {0, 0});
        ...
        {
            system("cls");
            //This draws a border box with ASCII block characters.
            for (int i = 0; i < 25; i++)
            {
                for (int j = 0; j < 70; j++)
                {
...
{   
    ...
    char selection;//Declares variables for menu navigation.
    int temp = 0;
    ...
    for (int i = 0;;)
    {
        ...
        if (_kbhit())
        {
            selection = _getch();//Checks if player pressed a key.
            ...
            if (selection == ' ')
            {
                ...
                break;//After finishing the game, show exit animation.
            }
            else if (selection == 'q')
            {
                system("cls");
...
}//Ensures screen doesn't scroll — keeps cursor at top.